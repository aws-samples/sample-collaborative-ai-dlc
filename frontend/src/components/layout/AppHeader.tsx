...
import {
    PanelLeftClose, PanelRightClose, PanelRightOpen, PanelLeftOpen,
    Search, ChevronRight, Settings, LogOut, Activity
} from 'lucide-react'
...
export function AppHeader({
    ...
    useEffect(() => {
        if (params.projectId) {
            projectsService.get(params.projectId)
                .then(setProject)
    ...
    useEffect(() => {
        if (params.projectId && params.sprintId) {
            sprintsService.get(params.projectId, params.sprintId)
                .then(setSprint)
    ...
    return (
        <header className="flex h-12 shrink-0 items-center border-b bg-background px-3 gap-2">
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={onToggleSidebar}>
                                {sidebarCollapsed ? <PanelLeftOpen className="h-4 w-4"/> :
            ...
            {/* Logo */}
            <button onClick={() => navigate('/dashboard')} className="flex items-center gap-1.5 shrink-0 group">
                <img src="/logo.svg" alt="AI-DLC" className="h-6 w-6"/>
                <span
                    className="text-sm font-semibold tracking-wide group-hover:text-foreground text-foreground/80 transition-colors">
          AI-DLC
        </span>
            </button>
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={onToggleActivity}>
                            {activityPanelOpen ? <PanelRightClose className="h-4 w-4"/> :
...
function buildBreadcrumbs(
    ...
    if (params.sprintId && params.projectId && sprint) {
        crumbs.push({label: sprint.name, href: `/project/${params.projectId}/sprint/${params.sprintId}`})
