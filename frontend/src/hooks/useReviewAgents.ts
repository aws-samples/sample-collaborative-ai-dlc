...
export function useReviewAgents({ projectId, sprintId }: UseReviewAgentsOptions) {
  ...
  const startBothReviews = useCallback(async (branch: string, baseBranch: string) => {
    if (!projectId || !sprintId) return;
    ...
    try {
      ...
      // Track mappings
      if (blindResult.executionId) execToType.current.set(blindResult.executionId, 'review-blind');
      if (blindResult.executionArn) execToType.current.set(blindResult.executionArn, 'review-blind');
  ...
  const resolveAgentType = useCallback((data: { executionId?: string; agentTaskId?: string; agentType?: string }): ReviewAgentType | null => {
  ...
  useEffect(() => {
    const unsubs = [
      ...
      realtimeService.on('agent.tool', (data) => {
        ...
        const setter = type === 'review-blind' ? setBlindAgent : type === 'review-full' ? setFullAgent : setModifyAgent;
        ...
        setter(prev => {
          ...
          if (isNewTool) {
            ...
            const idx = [...prev.toolCalls].reverse().findIndex(
      ...
      realtimeService.on('agent.tool_update', (data) => {
        ...
        setter(prev => ({
          ...
          toolCalls: prev.toolCalls.map(t =>
            t.id === data.toolCallId ? { ...t, status: data.status === 'error' ? 'failed' as const : 'running' as const } : t
          ),
        }));
      }),
      ...
      realtimeService.on('agent.completed', (data) => {
        ...
        if (type === 'review-blind') {
          setBlindAgent(prev => ({ ...prev, status: 'SUCCEEDED', activeToolCall: null, completedOutput: blindBuffer.current }));
        } else if (type === 'review-full') {
          setFullAgent(prev => ({ ...prev, status: 'SUCCEEDED', activeToolCall: null, completedOutput: fullBuffer.current }));
        } else if (type === 'review-modify') {
          setModifyAgent(prev => ({ ...prev, status: 'SUCCEEDED', activeToolCall: null, completedOutput: modifyBuffer.current }));
        }
      }),
  ...
  const isAnyRunning = blindAgent.status === 'RUNNING' || fullAgent.status === 'RUNNING' || modifyAgent.status === 'RUNNING';
