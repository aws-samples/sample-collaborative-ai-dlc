...
import {
  Search, Maximize2, X, Filter,
  FileText, BookOpen, ListChecks, Code2, HelpCircle,
  ShieldCheck, Info, GitPullRequest, Loader2,
  Network, LayoutGrid, Share2, Orbit, ChevronRight,
  Minus, Plus, Eye, EyeOff, Sparkles, ArrowRight, ArrowLeft,
  Map as MapIcon, BarChart3, Keyboard, Bot,
} from 'lucide-react';
...
const NODE_TYPES: Record<string, {
  ...
  Requirement: {
    ...
    textColor: '#fff', icon: FileText, label: 'Requirement', shortLabel: 'Req',
  },
  UserStory: {
    color: '#22c55e', darkColor: '#16a34a',
    gradientFrom: '#4ade80', gradientTo: '#16a34a',
    textColor: '#fff', icon: BookOpen, label: 'User Story', shortLabel: 'Story',
  },
  Task: {
    color: '#eab308', darkColor: '#ca8a04',
    gradientFrom: '#facc15', gradientTo: '#ca8a04',
    textColor: '#000', icon: ListChecks, label: 'Task', shortLabel: 'Task',
  },
  CodeFile: {
    color: '#ef4444', darkColor: '#dc2626',
    gradientFrom: '#f87171', gradientTo: '#dc2626',
    textColor: '#fff', icon: Code2, label: 'Code File', shortLabel: 'Code',
  },
  Review: {
    color: '#a855f7', darkColor: '#9333ea',
    gradientFrom: '#c084fc', gradientTo: '#9333ea',
    textColor: '#fff', icon: ShieldCheck, label: 'Review', shortLabel: 'Rev',
  },
  Question: {
    color: '#0ea5e9', darkColor: '#0284c7',
    gradientFrom: '#38bdf8', gradientTo: '#0284c7',
    textColor: '#fff', icon: HelpCircle, label: 'Question', shortLabel: 'Q',
  },
  GeneralInfo: {
    color: '#3b82f6', darkColor: '#2563eb',
    gradientFrom: '#60a5fa', gradientTo: '#2563eb',
    textColor: '#fff', icon: Info, label: 'General Info', shortLabel: 'Info',
  },
  PullRequest: {
    color: '#6366f1', darkColor: '#4f46e5',
    gradientFrom: '#818cf8', gradientTo: '#4f46e5',
    textColor: '#fff', icon: GitPullRequest, label: 'Pull Request', shortLabel: 'PR',
  },
  AgentRun: {
    color: '#64748b', darkColor: '#475569',
    gradientFrom: '#94a3b8', gradientTo: '#475569',
    textColor: '#fff', icon: Bot, label: 'Agent Run', shortLabel: 'Run',
  },
};
...
const TYPE_HIERARCHY: string[] = [
  'AgentRun', 'Requirement', 'GeneralInfo', 'UserStory', 'Task', 'CodeFile', 'Review', 'Question', 'PullRequest',
];
...
function hullPath(nodes: LayoutNode[], padding: number): string {
  ...
  return `M ${expanded[0].x} ${expanded[0].y} ` +
...
export default function SprintGraph() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setSettled(false);
    sprintGraphService.get(sprintId).then(({ nodes: n, edges: e }) => {
      ...
      if (layoutNodes.length > 0) {
        ...
        layoutNodes.forEach(nd => { nd.vx = 0; nd.vy = 0; });
      }
  ...
  const filteredNodeIds = useMemo(() => {
    ...
    nodes.forEach(n => {
      ...
      const matchesSearch = !search || n.label.toLowerCase().includes(search.toLowerCase()) || n.type.toLowerCase().includes(search.toLowerCase());
          ...
          for (let i = 0; i < neighborNodes.length; i++) {
            for (let j = i + 1; j < neighborNodes.length; j++) {
              ...
              if (rDist < minSep) {
                ...
                fa.fx -= rux * repel; fa.fy -= ruy * repel;
                fb.fx += rux * repel; fb.fy += ruy * repel;
              }
            }
          }
  ...
  const zoomIn = () => {
    const f = 0.75;
    setViewBox(v => ({ x: v.x + v.width * (1 - f) / 2, y: v.y + v.height * (1 - f) / 2, width: v.width * f, height: v.height * f }));
  };

  const zoomOut = () => {
    const f = 1.33;
    setViewBox(v => ({ x: v.x - v.width * (f - 1) / 2, y: v.y - v.height * (f - 1) / 2, width: v.width * f, height: v.height * f }));
  };
  ...
  const connectedEdgeKeys = useMemo(() => {
    ...
    edges.forEach((e, i) => { if (e.source === highlightNodeId || e.target === highlightNodeId) keys.add(i); });
    return keys;
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b bg-background/95 backdrop-blur-sm shrink-0 z-10">
        <div className="flex items-center gap-2 mr-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">Knowledge graph visualization</p>
          </div>
        </div>
            ...
            <TooltipTrigger asChild>
              <button
                ...
                className={cn(
                  ...
                  layoutMode === 'force' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'
                )}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={cn(
                  'flex items-center gap-1 rounded-sm px-2 py-1 text-[10px] font-medium transition-all',
                  layoutMode === 'hierarchical' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'
                )}
              >
        ...
        >
          ...
          Filters
          {typeFilters.size > 0 && (
            <Badge variant="default" className="h-4 px-1 text-[9px] ml-0.5">{typeFilters.size}</Badge>
          )}
        </Button>
        ...
        <div className="flex items-center gap-0.5">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={zoomIn}><Plus className="h-3.5 w-3.5" /></Button>
            </TooltipTrigger>
            <TooltipContent>Zoom in (+)</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={zoomOut}><Minus className="h-3.5 w-3.5" /></Button>
            </TooltipTrigger>
            <TooltipContent>Zoom out (-)</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={fitToContent}><Maximize2 className="h-3.5 w-3.5" /></Button>
            </TooltipTrigger>
            <TooltipContent>Fit to content (0)</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setShowKeyboardHelp(!showKeyboardHelp)}>
        ...
        <div className="flex items-center gap-1.5 px-4 py-1.5 border-b bg-muted/20 shrink-0 flex-wrap">
          ...
          {Object.entries(NODE_TYPES).map(([type, cfg]) => {
            ...
            return (
              ...
              >
                <span className="h-2.5 w-2.5 rounded-full shrink-0 ring-1 ring-black/10" style={{ backgroundColor: cfg.color }} />
                {cfg.label}
          ...
          {typeFilters.size > 0 && (
            <button onClick={() => setTypeFilters(new Set())} className="text-[10px] text-muted-foreground hover:text-foreground ml-2 underline underline-offset-2">
              Clear all
            </button>
          )}
        </div>
        ...
        {(loading || !settled) && nodes.length !== 0 ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 z-30">
            ...
            <div className="text-center">
              <p className="text-sm font-medium">{loading ? 'Loading graph' : 'Arranging layout'}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{loading ? 'Fetching sprint artifacts and relationships...' : 'Settling node positions...'}</p>
            </div>
          </div>
        ) : loading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Fetching sprint artifacts and relationships...</p>
            </div>
          </div>
        ) : nodes.length === 0 ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center max-w-xs">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Run the inception agent to generate requirements, user stories, and tasks.
                They'll appear here as an interactive knowledge graph.
              </p>
            </div>
            <Button
              variant="outline"
          ...
          <>
            ...
            >
              <defs>
                ...
                {Object.entries(NODE_TYPES).map(([type, cfg]) => (
                  <linearGradient key={`grad-${type}`} id={`grad-${type}`} x1="0" y1="0" x2="1" y2="1">
                ...
                <marker id="arrow-highlight" markerWidth="10" markerHeight="7" refX="10" refY="3.5" orient="auto">
              ...
              {/* ===== Cluster hulls ===== */}
              {showClusters && Object.entries(nodesByType).map(([type, typeNodes]) => {
              ...
              {filteredEdges.map((edge, i) => {
                ...
                return (
                  <g key={`e-${i}`} className="transition-opacity duration-300" opacity={dimmed ? 0.08 : 1}>
                    {/* Edge line */}
                    <line
                    ...
                    {!dimmed && (
                      <circle
                        cx={px} cy={py}
                        r={isHighlighted ? 3 : 2}
                        className={isHighlighted ? 'fill-foreground/40' : 'fill-muted-foreground/25'}
                      />
                    )}
                    ...
                    <text
                      x={mx} y={my - 8}
                      textAnchor="middle"
                      className={cn('text-[8px] fill-current select-none', isHighlighted ? 'text-foreground/50 font-semibold' : 'text-muted-foreground/25')}
                    >
              ...
              {nodes.filter(n => filteredNodeIds.has(n.id)).map(node => {
                ...
                return (
                  <g
                    ...
                    filter={isSelected ? 'url(#node-selected)' : isHovered ? 'url(#node-glow)' : 'url(#node-shadow)'}
                  >
                    {/* Selection ring */}
                    {isSelected && (
                      <rect
                        x={-NODE_W / 2 - 5} y={-NODE_H / 2 - 5}
                    ...
                    <rect
                      x={-NODE_W / 2 + 1} y={-NODE_H / 2 + 1}
                      width={NODE_W - 2} height={NODE_H / 2 - 1}
                      rx={NODE_RX - 1}
                      fill="white"
                      opacity={0.12}
                    />
              ...
              <div className="absolute bottom-3 left-3 z-10">
                <Card className="bg-background/90 backdrop-blur-sm shadow-lg">
                  <div className="px-2.5 py-1.5 border-b">
                    <span className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Legend</span>
                  </div>
                    ...
                    {Object.entries(NODE_TYPES).map(([type, cfg]) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] tabular-nums font-medium text-muted-foreground/60">{count}</span>
                        </button>
                      );
                    })}
                  </CardContent>
                ...
                <Card className="bg-background/90 backdrop-blur-sm shadow-lg">
                  <div className="px-3 py-2 border-b flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Graph Statistics</span>
                    </div>
                  ...
                  <CardContent className="p-3 space-y-2.5">
                    ...
                    {graphStats.hubNode && (
                      <>
                        <Separator />
                        <div>
                          <span className="text-[9px] uppercase font-medium text-muted-foreground tracking-wider">Hub Node</span>
                          <div className="flex items-center gap-1.5 mt-1">
                            <span
                              className="h-2.5 w-2.5 rounded shrink-0"
                              style={{ backgroundColor: NODE_TYPES[graphStats.hubNode.type]?.color || '#888' }}
                            />
                            <span className="text-[11px] font-medium truncate">{graphStats.hubNode.label}</span>
                    ...
                    <Separator />
                    <div>
                      <span className="text-[9px] uppercase font-medium text-muted-foreground tracking-wider">Edge Types</span>
                      <div className="mt-1 space-y-0.5">
                        {Object.entries(graphStats.edgeLabelCounts).sort((a, b) => b[1] - a[1]).map(([label, count]) => (
                          <div key={label} className="flex items-center justify-between">
                            <span className="text-[10px] text-muted-foreground">{EDGE_LABELS[label] || label}</span>
          ...
          <div className="absolute top-0 right-0 bottom-0 w-80 z-20 border-l bg-background/95 backdrop-blur-sm shadow-2xl flex flex-col">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  <div
                    ...
                    style={{ background: `linear-gradient(135deg, ${cfg?.gradientFrom || '#888'}, ${cfg?.gradientTo || '#666'})` }}
                  >
                    {(() => {
                      const Icon = cfg?.icon;
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: cfg?.color || '#888' }}>
                      {cfg?.label || selectedNodeData.type}
                    </span>
                    <h3 className="text-sm font-semibold mt-0.5 leading-snug">{selectedNodeData.label}</h3>
                  </div>
                </div>
                <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 -mt-1 -mr-1" onClick={() => setSelectedNode(null)}>
                  <X className="h-3.5 w-3.5" />
                </Button>
              </div>
            ...
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-5">
                ...
                {nodeEdges.length > 0 && (
                  <div>
                    <span className="text-[10px] uppercase font-semibold text-muted-foreground tracking-wider">Relationships</span>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <span className="text-[11px] truncate block font-medium">{other.label}</span>
                                  <span className="text-[9px] text-muted-foreground/60">{EDGE_LABELS[edge.label] || edge.label} &middot; {otherCfg?.label || other.type}</span>
                                </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold', STATUS_STYLES[status] || 'bg-muted text-muted-foreground')}>
      {display}
    </span>
  );
}
...
function DetailField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <span className="text-[10px] uppercase font-medium text-muted-foreground/60 tracking-wider">{label}</span>
...
function NodeDetailContent({ node }: { node: LayoutNode }) {
  ...
  switch (type) {
        ...
        <div className="space-y-3">
          {points && points !== '0' && (
            <DetailField label="Story Points">
              <div className="flex items-center gap-1.5">
                <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">{points}</span>
                <span className="text-[11px] text-muted-foreground">point{points !== '1' ? 's' : ''} estimated</span>
              </div>
            </DetailField>
          )}
          {description && (
        ...
        <div className="space-y-3">
          ...
          {depCount > 0 && (
            <DetailField label="Dependencies">
              <span className="text-xs text-muted-foreground">{depCount} task{depCount !== 1 ? 's' : ''} must complete first</span>
            </DetailField>
          )}
        ...
        <div className="space-y-3">
          {filePath && (
            <DetailField label="File Path">
              <code className="text-[11px] font-mono bg-muted/40 rounded px-1.5 py-0.5 break-all">{filePath}</code>
            </DetailField>
          )}
          {summary && (
          ...
          {commitRef && (
            <DetailField label="Commit">
              <code className="text-[10px] font-mono text-muted-foreground bg-muted/40 rounded px-1.5 py-0.5">{commitRef.slice(0, 12)}</code>
            </DetailField>
          )}
    ...
    case 'Question': {
      ...
      const parsedQuestions = tryParseJson(rawQuestions) as Array<{ text?: string; question?: string; type?: string; options?: Array<{ label: string; description?: string }> }> | null;
      const parsedAnswer = tryParseJson(rawAnswer) as { answers?: Array<{ selectedOptions?: number[]; freeText?: string }> } | null;
                ...
                return (
                  <div key={i} className="rounded-lg border overflow-hidden">
                    ...
                    <div className="bg-muted/30 px-3 py-2">
                      <span className="text-[9px] uppercase font-medium text-muted-foreground/60 tracking-wider">Question {parsedQuestions.length > 1 ? i + 1 : ''}</span>
                    ...
                    {hasAnswer && (
                      <div className="px-3 py-2 bg-emerald-500/5">
                        <span className="text-[9px] uppercase font-medium text-emerald-600 dark:text-emerald-400 tracking-wider">Answer</span>
                            ...
                            {selectedIdxs.map(idx => {
                              ...
                              return (
                                <div key={idx} className="flex items-start gap-1.5">
                                  <span className="text-emerald-600 dark:text-emerald-400 text-xs mt-0.5 shrink-0">&#10003;</span>
                                  <span className="text-xs">{opt?.label || `Option ${idx + 1}`}</span>
                                </div>
                              );
                            })}
                          </div>
                        ...
                        {freeText && (
                          <p className="text-xs leading-relaxed mt-1 text-foreground/80 italic">{freeText}</p>
                        )}
                      </div>
                    )}
                    ...
                    {!hasAnswer && (
                      <div className="px-3 py-2">
                        <span className="text-[10px] text-muted-foreground/50 italic">Awaiting answer</span>
                      </div>
                    )}
                  </div>
                );
        ...
        <div className="space-y-3">
          {infoType && infoType !== 'GeneralInfo' && (
            <DetailField label="Category">
              <Badge variant="outline" className="text-[10px]">{infoType}</Badge>
            </DetailField>
          )}
          {content && (
              ...
              <div className="flex items-center gap-1.5 text-[11px]">
                ...
                {baseBranch && (
                  <>
                    ...
                    <code className="font-mono bg-muted/40 rounded px-1.5 py-0.5">{baseBranch}</code>
                  </>
                )}
              </div>
            </DetailField>
    ...
    default: {
      ...
      const internalKeys = new Set(['id', 'type', 'label', 'x', 'y', 'vx', 'vy', 'pinned', 'sprint_id', 'createdAt', 'created_at']);
