...
export default function InceptionPage() {
    const {user} = useAuth();
    const {
        sprint, requirements, userStories, tasks, generalInfo, questions,
        projectId, sprintId, reload, getNeighbors,
    } = useSprint();

    const [startingAgent, setStartingAgent] = useState(false);
    const [startError, setStartError] = useState<AgentStartError | null>(null);
    const [aiModify, setAiModify] = useState<{ id: string; type: string; title: string } | null>(null);
    ...
    const currentUser = {id: user?.username || '', name: user?.displayName || user?.email || '', color: '#6366f1'};
    const {setActivity} = usePresence(sprintId, currentUser);

    const {description, setDescription, initDescription, synced, remoteUsers, setCursor} = useCollaborativeInception(
        projectId, user?.username || '', user?.displayName || user?.email || '',
        {
    ...
    useSprintEvents(sprintId, useCallback(() => {
        reload();
    }, [reload]));
    ...
    useEffect(() => {
        ...
        agentsService.getCurrentExecution(projectId, sprintId).then(exec => {
            if (exec?.executionArn) {
                ...
                if (exec.status !== 'FAILED' && exec.status !== 'ABORTED' && exec.status !== 'TIMED_OUT' && exec.status !== 'STOPPED') setHasLaunchedAgent(true);
            }
        }).catch(() => {
        });
    ...
    useEffect(() => {
        ...
        if (agentStatus.status?.status === 'FAILED' || agentStatus.status?.status === 'ABORTED' || agentStatus.status?.status === 'TIMED_OUT') {
        ...
        const terminal = agentStatus.status?.status === 'SUCCEEDED' || agentStatus.status?.status === 'FAILED' || agentStatus.status?.status === 'ABORTED' || agentStatus.status?.status === 'TIMED_OUT';
        if (terminal && pendingQuestions.length > 0) {
            ...
            Promise.all(pendingQuestions.map(q => questionsService.update(sprintId, q.id, {structuredAnswer: dismissed}).catch(() => {
            }))).then(() => reload());
        }
    ...
    const handleRerunAgent = async () => {
        ...
        try {
            const result = await agentsService.startWorkflow(projectId, {
                phase: 'inception', sprintId, description, changeRequest, event: {event: 'rerun'},
            });
            setExecutionArn(result.executionArn);
            setExecutionId(result.executionId || null);
            setHasLaunchedAgent(true);
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="max-w-4xl mx-auto p-6 space-y-6">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Inception Phase -- Define what you want to
                            build</p>
                    </div>
                    ...
                    {pendingQuestions.map(pq => (
                        <Card key={pq.id} className="border-agent-waiting bg-agent-waiting/5">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="outline"
                                               className="text-[10px] bg-agent-waiting/10 border-agent-waiting/30">Needs
                                            your input</Badge>
                                    </div>
                                    <Button variant="ghost" size="icon"
                                            className="h-6 w-6 text-muted-foreground hover:text-destructive"
                                            onClick={() => handleDismissQuestion(pq.id)} title="Dismiss question">
                            ...
                            <CardContent>
                                <QuestionEditor
                                    question={pq} sprintId={sprintId} userName={user?.displayName || user?.email || ''}
                                    onAnswer={(answer) => handleAnswerQuestion(pq.id, answer)}
                                    onAutoSave={async (draft) => {
                                        questionsService.update(sprintId, pq.id, {draftAnswer: draft}).catch(() => {
                                        });
                                    }}
                        ...
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    {synced ? (
                                        <Badge variant="outline"
                                               className="text-[9px] h-4 bg-agent-success/10 text-agent-success border-agent-success/30">Live</Badge>
                                    ) : (
                                        <Badge variant="outline" className="text-[9px] h-4">Connecting...</Badge>
                                    )}
                                </div>
                                {remoteUsers.size > 0 && <span
                                    className="text-[10px] text-muted-foreground">{remoteUsers.size} collaborator{remoteUsers.size > 1 ? 's' : ''}</span>}
                            </div>
                        </CardHeader>
                        <CardContent>
                            <CollaborativeTextarea
                                value={description} onChange={(val: string) => {
                                setDescription(val);
                                setActivity('description');
                            }}
                                onBlur={() => setActivity('idle')} onCursorChange={setCursor} remoteUsers={remoteUsers}
                                disabled={!promptEditable} placeholder="Describe what you want to build..."
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        {!hasLaunchedAgent ? (
                            isFirstRun ? (
                                <Button onClick={handleStartAgent} disabled={startingAgent || !description.trim()}
                                        className="gap-2">
                                    {startingAgent ? <Loader2 className="h-4 w-4 animate-spin"/> :
                                        <Play className="h-4 w-4"/>}
                                    {startingAgent ? 'Starting...' : 'Launch Inception Agent'}
                                </Button>
                            ) : (
                                <Button onClick={() => setShowRerunModal(true)} disabled={startingAgent}
                                ...
                                status={agentStatus.status?.status === 'RUNNING' ? 'running' : agentStatus.status?.status === 'SUCCEEDED' ? 'completed' : agentStatus.status?.status === 'FAILED' ? 'failed' : (sprint?.currentAgentStatus === 'cancelled' ? 'failed' : sprint?.currentAgentStatus) || 'idle'}
                                agentType="inception"
                            />
                        )}
                        {agentStatus.status?.status === 'SUCCEEDED' && hasArtifacts && sprint?.phase === 'INCEPTION' && (
                            <Button onClick={handleApprovePhase} disabled={approvingPhase} className="gap-2">
                                {approvingPhase ? <Loader2 className="h-4 w-4 animate-spin"/> :
                    ...
                    {/* Artifacts */}
                    {hasArtifacts && (
                        <Accordion type="multiple" defaultValue={['requirements', 'user-stories', 'tasks']}
                                   className="space-y-2">
                                ...
                                <AccordionTrigger className="py-3 hover:no-underline">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="secondary"
                                               className="h-5 px-1.5 text-[10px]">{requirements.length}</Badge>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent>
                                    <div className="space-y-2 pb-2">
                                        {requirements.map(req => (
                                            <ArtifactCard key={req.id} id={req.id} type="requirement" title={req.title}
                                                          fields={[
                            ...
                            <AccordionItem value="user-stories" className="border rounded-lg px-4">
                                <AccordionTrigger className="py-3 hover:no-underline">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="secondary"
                                               className="h-5 px-1.5 text-[10px]">{userStories.length}</Badge>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent>
                                    <div className="space-y-2 pb-2">
                                        {userStories.map(story => (
                                            <ArtifactCard key={story.id} id={story.id} type="user-story"
                                                          title={story.title}
                                                          fields={[
                            ...
                            <AccordionItem value="tasks" className="border rounded-lg px-4">
                                <AccordionTrigger className="py-3 hover:no-underline">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="secondary"
                                               className="h-5 px-1.5 text-[10px]">{tasks.length}</Badge>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent>
                                    <div className="space-y-2 pb-2">
                                        {tasks.map(task => (
                                            <ArtifactCard key={task.id} id={task.id} type="task" title={task.title}
                                                          status={task.status}
                                                          fields={[{
                                                              key: 'title',
                                                              label: 'Title',
                                ...
                                <AccordionItem value="general-info" className="border rounded-lg px-4">
                                    <AccordionTrigger className="py-3 hover:no-underline">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Badge variant="secondary"
                                                   className="h-5 px-1.5 text-[10px]">{generalInfo.length}</Badge>
                                        </div>
                                    </AccordionTrigger>
                                    <AccordionContent>
                                        <div className="space-y-2 pb-2">
                        ...
                        <Card>
                            <CardHeader className="pb-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <Badge variant="secondary"
                                           className="h-5 px-1.5 text-[10px]">{answeredQuestions.length}</Badge>
                                </div>
                            </CardHeader>
                            <CardContent>
                                                        ...
                                                        <div className="mt-1">
                                                            {q.structuredAnswer.answers[i].selectedOptions.map(optIdx => (
                                                                <Badge key={optIdx} variant="secondary"
                                                                       className="text-[10px] mr-1">{sq.options[optIdx]?.label}</Badge>
                                                            ))}
                                                            {q.structuredAnswer.answers[i].freeText &&
                                                                <p className="text-xs mt-1 italic">{q.structuredAnswer.answers[i].freeText}</p>}
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
            ...
            {aiModify && (
                <AiModifyModal artifactId={aiModify.id} artifactType={aiModify.type} artifactTitle={aiModify.title}
                               projectId={projectId} sprintId={sprintId} onClose={() => setAiModify(null)}
            ...
            <Dialog open={showRerunModal} onOpenChange={setShowRerunModal}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            The agent will review the existing artifacts and apply your changes. The original project
                            description is shown below for reference.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div>
                            <p className="text-xs font-medium text-muted-foreground mb-1">Original description</p>
                            <p className="text-xs border rounded-md p-2 bg-muted text-muted-foreground line-clamp-4">{description || '(no description)'}</p>
                        </div>
                        <div>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleRerunAgent} disabled={!changeRequest.trim() || startingAgent}
                                className="gap-2">
                            {startingAgent ? <Loader2 className="h-4 w-4 animate-spin"/> :
                                <RefreshCw className="h-4 w-4"/>}
                            Re-run Agent
            ...
            <AlertDialog open={showStartOver} onOpenChange={setShowStartOver}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle className="flex items-center gap-2"><AlertCircle
                            className="h-5 w-5 text-destructive"/> Start Over</AlertDialogTitle>
                        <AlertDialogDescription>This will delete all artifacts. Your inception prompt will be
                            preserved.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={handleStartOver}
                                           className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Start
                            Over</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
        </div>
