...
export async function fetchProjectInfos(l2: L2Intelligence): Promise<ProjectAgentInfo[]> {
  ...
  return Promise.all(allProjects.map(async (project) => {
    try {
      ...
      const latest = sprints.length > 0
        ? sprints.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0]
        : null;
      ...
      if (latest) {
        try {
          ...
          tasks.filter(n => (n as Record<string, unknown>).status === 'done')
        ...
        if (latest.phase === 'CONSTRUCTION') {
          try {
            ...
            taskStatuses = tasks;
            tasks.filter(t => t.executionStatus === 'SUCCEEDED')
