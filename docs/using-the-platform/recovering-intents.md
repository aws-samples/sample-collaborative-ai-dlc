...
| `kiro_session_missing`, `kiro_store_persist_failed` or `stage_park_persist_failed` | The CLI could not save a resumable conversation or its metadata. The unresumable question is retired and the stage fails with retry guidance; saved artifacts remain available.                                                          |
