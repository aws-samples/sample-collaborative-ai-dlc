...
On every WebSocket upgrade the Yjs server verifies the Cognito JWT and a short-lived HMAC-signed **scope token**, bound to that identity and the requested intent/sprint/project. It exchanges CRDT sync (`type 0`) and awareness (`type 1`) messages; a small persistence extension (`type 4`) confirms checkpoint completion. Unknown document-name formats are rejected. Sockets close at scope-token expiry (4401), and clients reconnect with jitter and fresh authorization, so a removed member can retain an existing session for at most the remaining token life.

Standalone mode keeps one process-local document owner and relies on browser REST autosaves for business-data persistence. Opt-in cluster mode distributes document owners across ECS workers using DynamoDB membership and fenced leases, forwards sockets between workers, and recovers binary Yjs checkpoints from S3 before synchronization. Workers checkpoint before idle eviction or ownership transfer. Manual sizing and automatic scaling are configured through `yjs_scaling`; a single hot document still has one owner. See [Yjs operations](../development/yjs-operations.md) for limits, rollout, recovery, metrics, load testing, and snapshot retention.
...
**Not shown.** Secrets Manager stores platform-wide provider OAuth application credentials and the optional GitHub App private key. SSM Parameter Store stores provider tokens, GitHub App configuration, and agent authentication material. Projects bind repository access through GitHub OAuth or GitHub App installations, GitLab OAuth, or Bitbucket OAuth. GitHub Issues, GitLab Issues, and Jira Cloud provide tracker imports and delivery comments; GitHub and GitLab issues also close after all final delivery requests merge. These integrations are omitted from the diagram to keep it readable; see [Git and Tracker Integration](../using-the-platform/git-integration.md) for their authentication and delivery behavior.
...
**Auth.** Bedrock and Kiro credentials live in SSM Parameter Store at personal, space, and platform scopes with `personal > space > platform` precedence. The control plane resolves the starting user's effective source and stores only an opaque binding with the intent; AgentCore reads that bound SecureString into an invocation-local environment, so long-lived sessions do not retain another user's secret. The binding never falls through after start: rotation at the same path is visible on the next invocation, while removal or invalidation produces an actionable credential failure. The runtime's IAM role deliberately has no Bedrock model-invocation permissions, so token auth is the only path. Git pushes use the starting user's provider token, injected only inside the engine's push/fetch windows and scrubbed from the checkout otherwise. None of these auth lookups appear in the diagram.
