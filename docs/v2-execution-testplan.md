...
### A7. (Optional) Real headless CLI against a local MCP server
Proves a real CLI actually drives our MCP tools (the integration test uses a fake
agent). Requires a Claude Code Bedrock bearer token and a reachable gremlin
container. This is a manual spot-check, not automated.
...
4. Run: `claude -p "List existing artifacts, then create a 'requirements-analysis'
   artifact id=demo." --mcp-config ./mcp-config.json --permission-mode bypassPermissions`
✅ The agent calls `lookup_artifacts` then `create_artifact`; the `Artifact`
vertex appears in gremlin anchored to the Intent.
...
- [ ] (Optional) A7 a real CLI drove the MCP tools.
