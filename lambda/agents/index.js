...
export const fetchRuntimeCapabilities = async (runtimeTarget = coreRuntimeTarget()) => {
  ...
  try {
    ...
    if (httpMethod === 'GET' && path.endsWith('/capabilities')) {
      ...
      if (event.queryStringParameters?.models !== '1') {
        const caps = await fetchRuntimeCapabilities(runtimeTarget);
      ...
      const [claudeModels, runtimeCaps] = await Promise.all([
        ...
        fetchRuntimeCapabilities(runtimeTarget),
      ]);
