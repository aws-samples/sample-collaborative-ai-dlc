...
| The learnings ritual / "capture a learning" / a reusable convention, decision, constraint, or gotcha that should steer FUTURE intents in this project | `record_team_knowledge({ id, title, content, agentRef })` with a stable kebab-case id. This is the team-knowledge tier; prior learnings are already shown to you above and re-readable via `get_team_knowledge`. NOT for this intent's outputs — those are `create_artifact`. |
