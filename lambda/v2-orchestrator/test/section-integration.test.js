...
const makeWorld = ({ remote, unitPlan, fileFor, beforeStage = null }) => {
  const world = {
    remote,
    invokes: [],
    sessions: [],
...
describe('WP8 fixture 2 — forced merge conflict → halt-and-ask SKIP (real git)', () => {
  it('the conflicting lane fails with the conflicted paths; skip completes the run; the intent branch stays pristine', async () => {
    ...
    // ── Lane verdicts: b MERGED; c FAILED at merge-lane with merge_conflict.
    ...
    expect(cFailed?.summary).toContain('merge_conflict');
    // The REAL merge-lane recorded the conflicted path, repo-qualified.
