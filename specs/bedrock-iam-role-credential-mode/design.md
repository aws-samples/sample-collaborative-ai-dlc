...
**Bedrock is an AWS service, so AWS Well-Architected and IAM best practice govern it: IAM roles with short-lived STS credentials are the preferred operating mode.** The existing Bedrock bearer-token mode remains as a deprecated compatibility path. Non-AWS model providers (Kiro today, LiteLLM later) use their own conventional pattern — an API key, plus a base URL where applicable — and are out of scope for the short-lived-credential rule.
...
| `con-cli-env-creds`            | Claude Code **2.1.246**, OpenCode **1.17.20**, and Codex **0.153.4** use the SDK-standard container provider through `AWS_CONTAINER_CREDENTIALS_FULL_URI` plus `AWS_CONTAINER_AUTHORIZATION_TOKEN`; no static STS variables or bearer token reach the role-mode CLI. Codex uses `amazon-bedrock-runtime`, signs for service `bedrock`, and requires no Codex-specific derived API key                                                                                              | Driver and active-stage tests; live Codex stage                                                                                                                                          |
...
| `con-codex-mantle`             | LEGACY. Codex 0.145.0 with `model_provider="amazon-bedrock"` calls `https://bedrock-mantle.<region>.api.aws/openai/v1/responses` and requires `bedrock-mantle:CreateInference` on `arn:aws:bedrock-mantle:<region>:<account>:project/*`. `bedrock:InvokeModel` does not authorize it. Superseded by `con-codex-runtime-provider`                                                                                                                                                   | Live probe (403 → 404 after granting)                                                                                                                                                    |
| `con-codex-model-missing`      | Mantle in eu-central-1 serves neither `openai.gpt-5.6-sol`, `global.openai.gpt-5.6-sol`, `gpt-5.6-sol` nor `anthropic.claude-sonnet-5` — each returns 404 _"The model does not exist"_. Mantle in `us-east-1` DOES serve the bare ids, so this is regional, not model-specific                                                                                                                                                                                                     | Live probe                                                                                                                                                                               |
| `con-codex-runtime-provider`   | Codex reaches Bedrock only through `model_provider="amazon-bedrock-runtime"` (needs >= 0.149.1; pinned 0.153.4), which signs SigV4 for service `bedrock` against `bedrock-runtime.<region>.amazonaws.com/openai/v1`. A custom provider cannot carry AWS auth (_"provider aws is only supported for `amazon-bedrock`"_), and overriding the legacy provider's `base_url` still signs for `bedrock-mantle` → _"401 Credential should be scoped to correct service: 'bedrock'"_       | Live probe on both pinned binaries                                                                                                                                                       |
| `con-codex-cris-required`      | On the runtime endpoint a bare `openai.gpt-5.6-sol` is refused (_"on-demand throughput isn't supported. Retry ... with the ID or ARN of an inference profile"_) while `global.openai.gpt-5.6-sol` succeeds — the exact opposite of Mantle, so a model id is only valid relative to a provider                                                                                                                                                                                      | Live probe                                                                                                                                                                               |
| `con-codex-project-grant`      | The OpenAI-compatible APIs authorize `bedrock:InvokeModel` against `arn:aws:bedrock:<region>:<account>:project/default` in addition to the model; without it every call returns 401 naming that resource                                                                                                                                                                                                                                                                           | Live probe with broker-minted credentials                                                                                                                                                |
| `con-codex-env-creds`          | The runtime provider resolves the invocation-scoped AWS container-credentials full URI/token and takes its Region from `AWS_REGION`; it needs no `AWS_PROFILE`, `~/.aws/config`, static STS variables, Bedrock API key, or derived Codex token                                                                                                                                                                                                                                     | Live probe                                                                                                                                                                               |
| `con-mmds-chain-live`          | AgentCore exposes execution-role credentials through its MicroVM metadata chain. `stage-materializer.js` scrubs every selected model credential and the invocation-specific full URI/token from all MCP children, then restores only non-static AgentCore runtime sources to the trusted `aidlc` bridge for DynamoDB and Neptune. Fail-closed inference therefore rests on the execution-role policy remaining free of Bedrock and STS permissions                                 | `lambda/agentcore/stage-materializer.js:455-478`; [AgentCore credentials management](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/security-credentials-management.html) |
| `con-custom-server-excluded`   | Every custom MCP server receives explicit empty values for selected-model credentials, refresh authority, and AgentCore runtime identity. The reserved `aidlc` entry is written last and restores only the runtime sources it needs                                                                                                                                                                                                                                                | `lambda/agentcore/stage-materializer.js:462-465`                                                                                                                                         |
...
| `con-one-binding-per-provider` | `resolveEffectiveCredentialBindings` resolves **exactly one** binding per provider. Kiro and bearer-only Bedrock use `user → space → platform`; a supported Bedrock role uses `space → platform` and is authoritative over covered bearer keys. A CLI therefore never sees both a bearer and a role credential                                                                                                                                                                     | `lambda/shared/agent-credentials.js:224`                                                                                                                                                 |
...
| `con-auth-context-seam`        | The orchestrator includes `bedrockRoleRefreshGrant` only on role-mode `run-stage-start`. AgentCore consumes that field to register the invocation-specific loopback route, deletes it before command execution, and closes over the registration for the detached stage job                                                                                                                                                                                                        | `lambda/agentcore/http-server.js:305-312`                                                                                                                                                |
...
| `con-pinned-versions`          | `CLAUDE_CODE_VERSION=2.1.246` (`lambda/agentcore/Dockerfile:36`), `OPENCODE_VERSION=1.17.20` (`:40`), `OPENCODE_SHA256=0a41572e…d86d9afc` (`:41`), `CODEX_VERSION=0.153.4` (`:53`). The OpenCode artifact tested was byte-identical to the pinned digest                                                                                                                                                                                                                           | Dockerfile + artifact inspection                                                                                                                                                         |
...
**Assumed, not verified.** That _billing_ aggregates by the chained session identity — AWS states the caller identity flows into Cost Explorer and CUR 2.0, but confirming it needs a CUR 2.0 caller-identity export and 24 hours, a customer-side action. Streaming attribution is not directly testable here (CLI 2.36.36 lacks `invoke-model-with-response-stream`, boto3 absent); prior live evidence recorded 23 `InvokeModel`/`InvokeModelWithResponseStream` events attributed to the final assumed-role session. Container-provider delivery, repeated refresh, revocation, and MCP isolation are covered by executable AgentCore and orchestrator integration tests rather than left as assumptions.
...
`AWS_CONTAINER_CREDENTIALS_FULL_URI` points to an invocation-scoped loopback endpoint and
`AWS_CONTAINER_AUTHORIZATION_TOKEN` carries its random authorization token. The top-level CLI also
receives the region its driver already sets. It receives no `AWS_ACCESS_KEY_ID`,
`AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, or `AWS_BEARER_TOKEN_BEDROCK`. MCP children receive
neither the endpoint nor the token.
...
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Credential expires mid-stage                               | The CLI refreshes through the loopback provider in the same process; the stage is not replayed                                                                                                                                                              | `req-expiry-failure-legible`       |
...
| `AssumeRole` denied, throttled or transient                | Initial resolution retains an allowlisted broker code and fails dispatch as `credential_resolution_failed`; active refresh returns a generic 503 and normally reconciles an exiting CLI as `cli_nonzero_exit`. Neither path automatically replays the stage | `req-broker-credential-resolution` |
...
| Supported Codex invoked                                    | Uses `amazon-bedrock-runtime`, a `global.openai.*` CRIS profile, and the `project/default` Runtime grant; no Mantle permission is used                                                                                                                      | `req-codex-scope`                  |
...
- **The grant is advice; the session policy is the enforcement.** The permission policy this deployment renders is attached to a role in an account it does not manage, so nothing verifies the operator attached it — or that they attached nothing wider. Since any role matching `bedrock_assumable_role_arns` that trusts the broker is bindable by a space owner, an over-permissive role would deliver its extra permissions verbatim into a stage container that runs model-authored code. The broker therefore attaches a **session policy** on every `AssumeRole`, making effective permissions the intersection. Measured 2026-09-07 against a throwaway role holding the reference grant plus `s3:ListAllMyBuckets`: without the ceiling the S3 call succeeded; with it the S3 call was denied while `eu.anthropic.claude-sonnet-5` still invoked and a bare foundation-model id stayed denied, so `con-fm-fence-works` survives the intersection. The ceiling and the grant are two renders of ONE statement definition (`terraform/bedrock-role-grant.tf`), differing only in wildcarding the account, because a ceiling that lags the grant denies a call the documented grant permits — which is exactly what a hand-copied ceiling would have done when Codex's runtime provider added the `project/default` statement. It renders to ~1 KB against the 2048-character inline session-policy limit. The ceiling is mandatory: a missing, empty, malformed, or over-broad value fails closed before `AssumeRole`, including capability checks and binding preflight, so deployment skew cannot silently mint credentials with the role's full permissions. This differs deliberately from an empty assumable-role allowlist because omitting an STS session policy widens authority rather than merely making eligibility unknown. It does not close the MMDS execution-role chain (`con-mmds-chain-live`), which remains bounded by that role's policy holding nothing useful.
...
- **Honest blast radius.** `con-mmds-chain-live` means the container credential chain still reaches the AgentCore execution role. That separate runtime identity is restored only to the trusted `aidlc` bridge for DynamoDB and Neptune and remains harmless for inference because its policy has no Bedrock or STS permission.
- **MCP isolation.** Every MCP child is explicitly scrubbed of static Bedrock credentials and the invocation-specific full URI/token. Custom children also lose runtime identity; the trusted bridge regains only the AgentCore relative-URI/token-file or web-identity inputs, never the selected model credential or refresh authority.
- **Confused deputy.** External ID mandatory cross-account, with the lifecycle in `req-external-id-lifecycle`.
- **Non-secret versus secret.** Neither the role ARN nor the external ID is a secret; AWS says so of the external ID in terms (`dec-external-id-not-secret`). Both are nonetheless **tenant-identifying**, so both are shown only on a read path gated to a principal that may modify the binding — which at every scope is the same principal that may overwrite it, so the gate costs nothing. The lower-privilege agent settings read, which any authenticated user can call, carries booleans only. The genuine secrets remain the bearer token and the Kiro key, which are never returned at any scope. `RoleSessionName` is non-secret but tenant-identifying, and appears in CloudTrail and logs by design.
...
The earlier Mantle probes are retained only as negative evidence for rejecting the legacy provider. The supported Codex path uses Bedrock Runtime and normal `bedrock:InvokeModel` authorization; no `bedrock-mantle` action is part of the grant, ceiling, or operational evidence path.
...
components:
  ...
  - id: auth-resolver
    ...
    description: Branches on result kind, then lets AgentCore replace initial STS material with loopback authority.
    responsibilities:
      - Scrub every credential and refresh variable from the base environment each invocation
      - Discard static STS values before a role-mode CLI starts
      - Set no bearer variable on the role path
    depends_on:
      - credential-broker
  - id: container-credentials-provider
    name: Invocation-scoped loopback credential provider
    description: Redeems the server-held refresh grant and returns non-cacheable AWS container credentials.
    responsibilities:
      - Bind only to loopback and require a distinct path and authorization token per invocation
      - Revoke the route and scrub mutable credential records when the stage ends
      - Keep refresh grants and STS values out of child-process environments
    depends_on:
      - credential-broker
      - grant-module
  - id: cli-drivers
    name: CLI drivers
    description: Every Bedrock driver forwards only the full URI and authorization token in role mode.
    responsibilities:
      - Use one SDK-standard provider contract without per-CLI credential files or helpers
  ...
  - id: settings-api
    ...
    responsibilities:
      ...
      - Generate the external ID, return it only on a read gated to a principal that may modify the binding, and expose only its set state on lower-privilege reads
  ...
  - id: grant-module
    ...
    description: Keeps the invocation grant and adds a separate audience for bounded role refresh.
    responsibilities:
      - Keep the invocation grant at 300s and one-shot
      - Bind refresh authority to one active stage through its callback deadline
    depends_on: []
  - id: stage-failure-path
    name: Stage credential-refresh failure path
    description: >-
      Initial resolution failures remain typed. During a running attempt the loopback provider
      reduces broker failures to a sanitized 503; if the CLI exits, reconciliation normally records
      cli_nonzero_exit and never automatically replays the stage.
    responsibilities:
      - Preserve credential_resolution_failed for initial dispatch failures
      - Keep the active-refresh broker code in allowlisted broker logs while returning only the generic 503
      - Never replay completed stage work as an automatic credential recovery
    depends_on:
      - auth-resolver
```
...
```yaml
decisions:
  - id: dec-mid-stage-refresh
    title: Role credentials refresh inside the active stage
    status: accepted
    context: >-
      STS role chaining hard-caps each session at 3600 seconds while one stage may run for eight hours.
      Replaying a whole stage can duplicate side effects and is not an implemented recovery path.
    decision: Use a token-protected loopback container-credentials endpoint backed by a bounded, audience-separated refresh grant.
    consequences: >-
      The CLI can cross multiple credential expirations without restarting. Refresh authority is
      revoked and scrubbed at invocation end and cannot be redeemed for a bearer token.
  - id: dec-credential-delivery
    title: SDK container-credentials provider, not static credential variables
    status: accepted
    context: >-
      Every supported CLI already uses the AWS SDK provider chain. Static environment credentials
      cannot rotate during one long-running process.
    decision: Set the standard container-provider URI and authorization token per invocation and install nothing in the image.
    consequences: >-
      Credentials stay out of the CLI environment, and one provider contract works across drivers.
  ...
  - id: dec-codex-scope
    ...
    consequences: >-
      Supported Codex versions work in role mode without a legacy Mantle rollback permission.
      Because the available GPT profiles are global CRIS profiles, administrators must decide whether
      their cross-Region routing is compatible with the deployment's data-residency policy.
  ...
  - id: dec-external-id-scope
    ...
    consequences: The simplest single-account deployment manages no additional value, and none is generated for it.
  - id: dec-external-id-not-secret
    title: The external ID is platform-generated, non-secret, and re-readable rather than shown once
    status: accepted
    context: >-
      req-external-id-lifecycle originally required the value never be returned to a client, which
      contradicted the bootstrap order in req-same-and-cross-account: an operator cannot write
      sts:ExternalId into a trust policy without seeing it. Resolving it needed AWS's own position
      rather than a preference. AWS states that the value "must be generated by Example Corp and not
      their customers", and separately that "AWS does not treat the external ID as a secret ... The
      external ID for a role can be seen by anyone with permission to view the role", contrasting it
      with an access key which cannot be viewed again
      (https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html).
      The protective property is that the ASSUMING party controls the value, not that it is hidden:
      the trust policy names the broker role as its only principal, so role ARN plus external ID is
      useless to any caller that is not the broker. Verified empirically that STS accepts the full
      documented charset including the underscore that base64url emits, an AssumeRole probe returning
      AccessDenied rather than ValidationError for each of the symbol classes, so the generator cannot
      produce a value the API rejects even though the user-guide prose omits underscore.
    decision: >-
      Generate the value platform-side, never accept a client-supplied one, preserve it across saves,
      and return it idempotently on any read gated to a principal that may modify the binding. Reject
      the show-once pattern and never treat an ordinary binding save as rotation.
    consequences: >-
      Bootstrap, disaster recovery and later IaC reconciliation are plain reads. The release has no
      in-place rotation operation: a space gets a different value only when the entire space is deleted
      and recreated, which also deletes all space-scoped agent credentials; platform scope has no
      supported replacement operation. A future in-place workflow must be an explicit
      platform-admin-gated operation that coordinates generation, trust-policy update, binding
      replacement and preflight. Direct SSM mutation is unsupported because it can leave the binding
      carrying a different value. The residual read exposure is to a principal that can already
      overwrite the binding, so nothing is weakened. Operator-supplied external IDs are rejected
      outright as contrary to AWS guidance. AWS recommends one external ID per AWS account, which
      coincides with one platform-scope binding but diverges when many spaces share one role -- there
      the operator must enumerate each space's value in one trust policy, so a shared role should
      prefer a single platform-scope binding and use the session-name condition for per-space scope.
  - id: dec-external-id-storage
    ...
    context: >-
      ...
      the binding, the first (necessarily failing) cross-account save would discard it and the next
      save would generate a different one, so the trust policy the operator had just written would
      already be stale and the bootstrap would never converge.
    decision: >-
      Generate the value idempotently into a dedicated per-scope parameter — {prefix}/bedrock-external-id
      and {prefix}/projects/<projectId>/bedrock-external-id — never overwrite it during a save, and
      copy it into the binding value when a save succeeds. Delete the space value only as part of
      deleting the entire space. The binding remains the single value the broker reads, so resolution
      is unchanged.
    consequences: >-
      The value is stable across rejected and successful saves, role-ARN changes and temporary
      removal of the binding, so the two-attempt cross-account bootstrap converges and recovery is a
      plain read. The parameter deliberately sits OUTSIDE the agent-credentials paths: the external
      ID is not a credential, so the settings API can read its own generated value back without
      gaining any read permission on credential material, and the broker's exclusive-read discipline
      for credentials is untouched. Costs one new SSM path and a small IAM statement. The value can
      outlive the binding it was made for; it is inert, and reusing it on a later binding for the same
      scope is the intended behaviour rather than an implicit rotation.
  ...
  - id: dec-binding-preflight
    ...
    context: >-
      ...
      outside the allowlist is discovered by a failing stage with no automatic replay and reports a
      typed reason with no provider detail by design. Cross-account
...
1. **In-container credential delivery** — Claude Code, OpenCode, and Codex each receive only the invocation-specific container-provider URI/token and complete Bedrock calls without static STS variables.
2. **Fail-closed and isolated** — with resolution returning nothing or refresh authorization invalid, the stage fails; no MCP child receives the selected Bedrock endpoint/token, and the trusted bridge cannot invoke Bedrock under the execution-role identity.
...
11. **Expiry and resume** — a forced short credential is refreshed repeatedly inside one stage attempt, and a multi-hour gate wait persists no temporary credentials before the next stage obtains a fresh grant, endpoint token, and role session.
